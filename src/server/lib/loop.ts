...
/** The only tools accepted during the wrap-up reserve. */
...
export function isSuccessfulActionResult(kind: ToolCallKind, name: string, result: string): boolean {
  ...
  return !FAILED_ACTION_PREFIXES.some(p => result.startsWith(p))
}
...
const WRAPUP_NOTE_CAP =
  ...
  `Everything you just learned is about to be LOST: the next turn starts from the TODO and ` +
  `memory shown in your prompt, which are now out of date.\n\n` +
...
const WRAPUP_NOTE_ACTION =
  `✅ ACTION DONE — this turn ends here; one action per turn. You have not recorded anything ` +
  ...
  `Write down what you verified, what you just did, and the single next action — so the next ` +
  `turn resumes instead of re-deriving all of this.`
...
export async function runAgentTurn(
  ...
  const injectWrapUp = (reason: 'cap' | 'action') => {
    ...
    log('system', reason === 'cap'
      ? `Wrap-up reserve: ${rounds}/${maxRounds} rounds used with no state write — only update_todo/update_memory are accepted from here`
      : `Wrap-up reserve: action completed after ${rounds} round(s) with no state write — only update_todo/update_memory are accepted from here`)
  }
  ...
  while (true) {
    if (wrapUpInjected) {
      ...
      // has persisted, or when the reserve is spent.
      if (wroteState || rounds >= reserveEndsAt) break
    ...
    await compactContext(summaryModel, context, compaction, options, model)
    ...
    for (const toolCall of toolCalls) {
      ...
      if (STATE_WRITE_TOOLS.has(toolCall.name)) wroteState = true
    ...
    if (actionPending || actionSucceeded) {
      ...
      if (endTurnAfterAction) {
        if (wroteState) {
          log('system', `${what} — state already recorded this turn, ending turn`)
          return 'completed'
        }
        if (!wrapUpInjected) {
          injectWrapUp('action')
    ...
    // Cap path: one round before the ceiling, with nothing persisted, open the reserve.
  ...
  if (wrapUpInjected) {
    ...
    if (wroteState) {
      log('system', `Turn ending after ${opened} — state recorded in the wrap-up reserve (${rounds} rounds)`)
    } else {
      log('system', `Turn ending after ${opened} — wrap-up reserve exhausted with NO state write this turn; next turn resumes from unchanged TODO/memory`)
    }
  } else if (wroteState) {
