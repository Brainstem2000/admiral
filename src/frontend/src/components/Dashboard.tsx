...
import { Settings, Sun, Moon, Github, AlertTriangle, CircleHelp, Globe, BarChart3, Coins, Vault, Users, User, BookOpen, Radar, Flag, Warehouse} from 'lucide-react'
...
import { FactionPane } from './FactionPane'
import { FactionVaultPane } from './FactionVaultPane'
...
export function Dashboard({ profiles: initialProfiles, providers, registrationCode, gameserverUrl, defaultProvider, defaultModel, onRefresh, onShowProviders }: Props) {
  ...
  const [view, setView] = useState<'profiles' | 'character' | 'map' | 'intel' | 'analytics' | 'codex' | 'faction' | 'vault'>(() => {
      ...
      <div className="sticky top-0 z-50 flex items-center justify-between h-12 px-3.5 bg-card border-b border-border">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center border border-border divide-x divide-border">
            ...
            <NavButton active={view === 'faction'} onClick={() => setView('faction')} icon={<Flag size={12} />} label="Faction" title="Faction overview: treasury, members, lockbox storage" />
            <NavButton active={view === 'vault'} onClick={() => setView('vault')} icon={<Warehouse size={12} />} label="Vault" title="Faction vault: full shared inventory, and what the open builds still need" />
